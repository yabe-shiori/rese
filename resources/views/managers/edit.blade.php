    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="container mx-auto mt-8">
            <div class="max-w-md mx-auto bg-white rounded p-8">
                <h2 class="text-2xl font-semibold mb-6">Edit Shop</h2>
                ...
                <form method="POST" action="{{ route('managers.update', $shop->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="description" class="form-textarea w-full" name="description" required>{{ old('description', $shop->description) }}</textarea>
                    </div>
                        ...
                        <select id="genre_id" name="genre_id" class="form-select w-full" required>
                            @foreach ($genres as $genre)
                                <option value="{{ $genre->id }}"
                                    {{ $shop->genre_id == $genre->id ? 'selected' : '' }}>{{ $genre->name }}</option>
                            @endforeach
                        </select>
                    </div>
